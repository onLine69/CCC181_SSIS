...
class StudentForm(FlaskForm):
    id_number = StringField(label="ID Number", validators=[validators.DataRequired(message="Provide an ID number e.g. 2022-3337"), validators.length(min=9,max=9,message="Length not valid, should be equal to 9."), 
    ...
    gender = StringField(label="Gender", validators=[validators.DataRequired(message="Must have an input."), validators.length(min=1,max=50,message="Length not valid.")])